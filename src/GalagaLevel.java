...
/**
 * Abstract base class for Galaga-style levels.
 * Handles player movement, bullet firing, collision detection, and enemy behavior.
 * Concrete subclasses define specific enemy layouts and difficulty.
 */
...
public class GalagaLevel extends Level {
	...
	public int step(KeyCode currentKey) {
		...
		// ️⃣ Check collisions between bullets and enemies
		...
		// ️⃣ Check lose condition: any enemy reaches player's Y
		for (Enemy e : enemies) {
			double enemyBottom = ((javafx.scene.image.ImageView) e.getView()).getY()
					+ ((javafx.scene.image.ImageView) e.getView()).getFitHeight();
